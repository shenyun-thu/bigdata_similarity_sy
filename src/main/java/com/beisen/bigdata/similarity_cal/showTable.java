...
public class showTable {
    ...
    private static double max_limit = 3;//各个维度上对应的差值最大限度
    private static int nums_limit = 10;//维度数量限制
    public static void main(final String[] args) throws Exception {
        
        
        SparkConf sc = new SparkConf();
        sc.setAppName("showTable");
        JavaSparkContext jsc = new JavaSparkContext(sc);
        Scan scan = new Scan();
        Configuration conf = SparkUtil.buildHbaseConfig("BEISENTALENTDW.ASSESSMENT_RUITUUSERRESULTINFO",scan,IS_ONLINE);
        ...
        talentRdd.groupByKey().foreachPartition(new VoidFunction<Iterator<Tuple2<String, Iterable<String>>>>() {
            ...
            public void call(Iterator<Tuple2<String, Iterable<String>>> f) throws Exception {
                ...
                BufferedMutator mutator = HbaseUtil.getMutator(conn,"beisendw:talentSimilarity_assessment_ruituuserresultinfo");
                try{
                    while (f.hasNext()) {
                        ...
                        for (int i = 0,len = p.size();i < len;i++) {
                            if(p.get(i).average < average_score_limit || p.get(i).num < nums_limit){
                                p.remove(i);
                                --len;
                                --i;
                            }
                            ...
                            for (int j = i + 1; j < p.size(); j++) {
                                ...
                                if (can_cal) {
                                    ...
                                    if(similarity_percent > similarity_limit){
                                        ...
                                        put.addColumn("fmy".getBytes(),"weidu".getBytes(),(p.get(i).num + " ge").getBytes());
                                        put.addColumn("fmy".getBytes(),"average".getBytes(),(p.get(i).average + " " +p.get(j).average+" ").getBytes());
                                        put.addColumn("fmy".getBytes(),"max_".getBytes(),(max_temp+" ").getBytes());
                                        mutator.mutate(put);
                                    }
                                }
                            }
